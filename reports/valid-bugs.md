# Validated findings (running count 0)

- 3 lead(s) marked VALID at 2026-09-06 04:10:10 UTC
  - **VERDICT: VALID**
  - | Seller Data Hub /public/ exposure | **VALID** | Unauthenticated seller registry + CORS:ACAC across 6 countries |
  - **Valid findings to report:** Lead 1 (Seller Data Hub) is the sole VALID finding. Submit via bugs.olivermaicher.eu with PoC demonstrating unauthenticated access to `/public/de/feature-toggle` and `/pu

- 2 lead(s) marked VALID at 2026-09-06 12:49:07 UTC
  - **Verdict: VALID**
  - | seller-data-hub `/public/` + CORS ACAC | **VALID** | Unauthenticated seller PII registry, 6-country enumeration, any-origin credentialed reads |

- 3 lead(s) marked VALID at 2026-09-08 20:19:09 UTC
  - **Verdict: VALID**
  - | Q4 Provable | NO — Requires valid seller clientId/clientSecret. Cannot prove without credentials. |
  - | 1. Seller Data Hub Public Registry + CORS ACAC | **VALID** | 5.3 MEDIUM | Submit to bugs.olivermaicher.eu |

- 1 lead(s) marked VALID at 2026-09-17 22:55:26 UTC
  - **VALID (2):**
